...
- ✅ **`HeroState.LearnSkill`** : relie enfin le modèle de données à la vraie logique de jeu — dépense un point de compétence pour apprendre n'importe quel sort du registre `DevilSkills` (pas seulement Trait de feu), avec toutes les vérifications (palier via `CanLearnSkill`, points disponibles, pas déjà connu). `NewTraitDeFeuSkill` généralisé en `NewDevilHeroSkill(skillID)`. Pas encore de vrai appelant : rien ne donne de points de compétence tant qu'il n'y a pas de système de niveau/XP. Commit `4a85e2f7`.
