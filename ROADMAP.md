...
Les items restants (modificateurs d'équipement, résistances, bouclier de mana) partagent tous la même dépendance : soit une assignation de personnage/objets Devil qui n'existe pas encore (Phase 2), soit des monstres qui attaquent (Phase 4, IA). Phase 1 a maintenant épuisé ce qui est testable en isolation côté serveur ; la suite logique est d'avancer sur la Phase 2 (arbre Élémentalisme) ou la Phase 4 (IA de monstre) plutôt que de continuer à empiler des mécanismes non atteignables en jeu.
