...
*Démarrée — 9/10 sorts d'Élémentalisme, 8/10 Arcane, 1/10 Ésotérisme.*
...
- ✅ **Vortex** (7e sort d'Arcane, palier 5/niveau 24) : "Aspire toutes les entités proches vers un point" — l'inverse de Télékinésie. Nouvelle méthode `d2mapentity.NPC.Pull(cible, distance)`, symétrique de `Knockback` mais bornée pour ne jamais dépasser la cible (contrairement à `Knockback`, un no-op si le PNJ est déjà sur la cible plutôt qu'un fallback directionnel). Sort d'Arcane du plus haut palier implémenté à ce stade. Même limite documentée que Télékinésie : aucun paquet ne diffuse encore la nouvelle position aux clients. Commit `10758f14`.
...
- Reste 1 sort d'Élémentalisme (Maîtrise élémentaire — passif, bloqué sur l'investissement multi-points), 2 sorts d'Arcane (Rupture arcane — bloqué tant qu'aucune donnée de résistance de monstre n'existe ; Résonance magique — passif de synergie, même limite), 9 sorts d'Ésotérisme — les invocations (Familier, Golem arcane) réutiliseront `d2mapentity.NPC` côté allié, pas ennemi.
